---
description: "Serves packs from local directories: the pack.yml manifest, the rules/ and skills/ layout, root precedence, and the composition rows a pack produces."
...
Rules come before skills. Only `.md` files directly under `rules/` are read — no recursion — and a file over `maxRuleBytes` is skipped rather than truncated, because half an instruction is worse guidance than none. A pack shipping neither still lists: binding it is a meaningful action once it gains contents.

A composition that mounts bound packs must also have `dsh-pack-rules` and `dsh-skill-filesystem` resolvable, since those are the module names the generated rows carry.
...
Nothing directly. This package produces composition rows; what a mounted row contributes is that row's own contract — `dsh-pack-rules` for the prompt sections a pack's rules become, and `dsh-skill-filesystem` for the skills its catalog publishes. Discovery, manifests, and root precedence never reach a request.

#### Token effect

Zero directly. The rows this package emits carry the cost: rules add their text to every request in the bound scope, and skills add their name and description to the session's skill catalog.
...
- **Only rules and skills are read** — a pack's `hooks/`, `commands/`, `agents/`, and `mcp.json` are ignored: those loaders do not exist yet, so shipping them in a pack silently contributes nothing.
...
- **Nothing verifies the generated row modules resolve** — a composition missing `dsh-pack-rules` or `dsh-skill-filesystem` fails when a bound pack mounts, not when its pack is listed here.

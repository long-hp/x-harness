...
| `session/new` | A fresh persistent agent whose absolute workspace and stdio or HTTP MCP servers are validated before publication, composed with that workspace's bound packs, plus its complete configuration-option state. |

...
Loaders for a pack's `commands/`, `agents/`, and `mcp.json` are unbuilt; `dsh-pack-local` reads `rules/`, `skills/`, and `hooks/hooks.json` only, so shipping the others in a pack contributes nothing today. A pack's hooks also reach only the seven Claude Code events the bridge implements. The `pack` Remote namespace exists, but no shipped surface calls it: binding is still a scripted call rather than something a user clicks.

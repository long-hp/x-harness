...
flowchart LR
  ...
  pkg_api_pack_controller["api-pack-controller"]
  svc_packController["ctx.packController<br/>Host pack Remote controller"]
  ...
  pkg_api_pack_controller --> svc_packController
...
| `ctx.packController` | `core` | [`api-pack-controller`](../packages/api/pack-controller) | - | - | - | 把 pack 注册表与绑定存储投影到生成的 Remote namespace 上：请求准入、扣下提供方宿主路径的浏览器安全摘要视图，以及拒绝映射都位于此处，而不在任一 seam Definition 上。 |
